...
let make'
  ...
  and visit' pkgId pkg build =
    ...
    let dist, sourceType =
      ...
      | EsyInstall.Solution.Package.Include _
...
let buildTask ?quiet ?buildOnly ?logPath ~cfg task =
  Logs_lwt.debug (fun m -> m "build %a" PackageId.pp task.Task.pkg.id);%lwt
  let plan = Task.plan task in
  let label = Fmt.strf "build %a" PackageId.pp task.Task.pkg.id in
...
let buildDependencies' ~concurrency ~buildIncluded ~cfg plan id =
  let open RunAsync.Syntax in
  ...
  let runIfNeeded changesInDependencies task =
    ...
    match buildIncluded, task.pkg.source with
    | false, Package.Include _ -> return Changes.No
    | _ ->
...
let buildDependencies ?(concurrency=1) ~buildIncluded ~cfg plan id =
  Perf.measureLwt
    ~label:"buildDependencies"
    (fun () ->buildDependencies' ~concurrency ~buildIncluded ~cfg plan id)
